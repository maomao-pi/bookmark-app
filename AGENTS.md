...
系统支持**多模型配置**，不同 AI 功能可使用不同的模型。配置分为两类：

| 类型 | 配置前缀 | 用途 |
|------|---------|------|
| 文本模型 | `ai.text.*` | 文本生成、描述生成、标签建议等 |
| 联网搜索模型 | `ai.search.*` | AI 推荐、联网搜索等需要实时信息的场景 |
...
| 键名 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `ai.text.enabled` | boolean | 是否启用文本 AI 功能 | `true` |
| `ai.text.apiKey` | string | 阿里云百炼 API Key | - |
| `ai.text.baseUrl` | string | API 地址 | `https://dashscope.aliyuncs.com/compatible-mode/v1` |
| `ai.text.model` | string | 模型名称 | `qwen3-plus` |

**联网搜索模型配置**（`ai.search.*`）：

| 键名 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `ai.search.enabled` | boolean | 是否启用联网搜索 AI 功能 | `false` |
| `ai.search.apiKey` | string | 阿里云百炼 API Key（可与文本模型相同或不同） | - |
| `ai.search.baseUrl` | string | API 地址 | `https://dashscope.aliyuncs.com/compatible-mode/v1` |
| `ai.search.model` | string | 模型名称（如 `qwen3-search`） | - |

**其他 AI 配置**：

| 键名 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `ai.organizeEnabled` | boolean | 启用 AI 整理建议 | `true` |
| `ai.organizeFrequencyDays` | number | 整理建议生成频率（天） | `7` |
| `ai.autoGenerateDescription` | boolean | 自动生成描述 | `false` |
| `ai.autoExtract` | boolean | 自动提取标签 | `true` |
| `ai.timeout` | number | AI 请求超时（毫秒） | `15000` |
...
| 场景 | 推荐模型 | 说明 |
|------|---------|------|
| 文本生成（免费） | `qwen3-plus` | 文本理解与生成能力强 |
| 文本生成（付费） | `qwen3-max` | 最强文本能力 |
| 联网搜索 | `qwen3-search` | 支持实时联网搜索，返回真实链接 |
| 联网搜索（高并发） | `qwen3-search-pro` | 高并发联网搜索 |

### 11.4 配置示例

```json
{
  "ai.text.enabled": "true",
  "ai.text.apiKey": "sk-xxx",
  "ai.text.baseUrl": "https://dashscope.aliyuncs.com/compatible-mode/v1",
  "ai.text.model": "qwen3-plus",
  "ai.search.enabled": "true",
  "ai.search.apiKey": "sk-xxx",
  "ai.search.baseUrl": "https://dashscope.aliyuncs.com/compatible-mode/v1",
  "ai.search.model": "qwen3-search",
  "ai.organizeEnabled": "true",
  "ai.organizeFrequencyDays": "7"
}
```

### 11.5 开发注意事项

1. **向后兼容**：旧配置 `ai.apiKey`、`ai.model`、`ai.baseUrl` 仍可使用，系统会自动兼容
2. **缓存清除**：修改 AI 相关配置后，系统会自动清除 AI 推荐缓存
3. **分开配置**：建议文本模型和联网搜索模型使用不同的 API Key，便于分别计费和监控
