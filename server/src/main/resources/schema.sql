...
CREATE TABLE IF NOT EXISTS app_user (
    ...
    role VARCHAR(20) DEFAULT 'user' COMMENT '角色: admin(可管理), user(普通用户)',
    status VARCHAR(20) DEFAULT 'active' COMMENT '状态: active, disabled',
    permissions TEXT DEFAULT NULL COMMENT '权限模块JSON，格式: {"menus":{"dashboard":true,"users":true,...},"tabs":{"bookmarks":{"list":true,"add":true},...}}',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    last_login_at DATETIME DEFAULT NULL COMMENT '最后登录时间',
    deleted TINYINT DEFAULT 0 COMMENT '逻辑删除: 0-未删除, 1-已删除'
...
CREATE TABLE IF NOT EXISTS bookmark (
    ...
    favicon LONGTEXT DEFAULT NULL COMMENT '网站图标',
...
('ai.apiKey', '', 'ai', '文本 AI API Key（Minimax / 百炼 / 智谱等）'),
('ai.baseUrl', 'https://api.minimaxi.com/anthropic', 'ai', '文本 AI 基址（Minimax Anthropic 兼容）'),
('ai.model', 'MiniMax-M2.7', 'ai', '文本 AI 模型'),
...
('ai.search.enabled', 'false', 'ai', '联网搜索 AI 开关'),
('ai.search.apiKey', '', 'ai', '联网搜索 AI API Key（Minimax 等）'),
('ai.search.baseUrl', 'https://api.minimaxi.com/anthropic', 'ai', '联网搜索 AI 基址（Minimax Anthropic 兼容）'),
('ai.search.model', 'MiniMax-M2.7', 'ai', '联网搜索 AI 模型'),
